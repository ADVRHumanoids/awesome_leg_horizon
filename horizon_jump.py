...
# "Loading the "URDF"
urdf_path = "/home/andreap/IIT_CRIS_HHCM/ROS/ROS1_noetic/awesome_leg_ws/src/awesome_leg_description/urdf/awesome_leg_complete.urdf"
...
dfk_foot = cs.Function.deserialize(urdf_awesome_leg.frameVelocity("LF_FOOT", casadi_kin_dyn.py3casadi_kin_dyn.CasadiKinDyn.LOCAL_WORLD_ALIGNED))
...
prb.createConstraint("foot_vel_bf_touchdown", v_LF_FOOT,nodes=range(0,n_takeoff+1)) # no vertical velocity of the foot before touchdown
prb.createConstraint("foot_vel_aftr_touchdown", v_LF_FOOT,nodes=range(n_touchdown,n_nodes+1)) # no vertical velocity of the foot after touchdown

prb.createConstraint("foot_pos_restoration", p_LF_FOOT-p_LF_FOOT_init,nodes=n_nodes) # no vertical velocity of the foot after touchdown

prb.createConstraint("GRF_zero", f_contact,nodes=range(n_takeoff,n_touchdown)) # no vertical velocity of the foot after touchdown
...
rpl_traj = replay_trajectory(dt, joint_names, solution["q_p"]) # replaying the trajectory and the forces on (it publishes on ROS topics)
